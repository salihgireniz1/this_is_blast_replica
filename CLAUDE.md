...
**Where it stands (2026-09-07): the case is feature-complete. Unity suite 103/103, level
...
- **Presentation** — `LevelSpawner` (view registry + every layout number as structs),
  ...
  `FlightPath` (segment-vs-footprint: a bullet brushes the front cubes its line crosses, polled per flight frame, `CubeView.Nudge` leans them by `GameDirector.Brush`'s curve).
