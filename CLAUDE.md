...
- **Presentation** — `LevelSpawner` (view registry + every layout number as structs),
  ...
  `FlightPath` (segment-vs-footprint: a bullet brushes the front cubes its line crosses, polled per flight frame; `CubeView.Nudge` swings them about the contact point and pushes them away from it by `GameDirector.Brush`'s curve, on the layer the bullet flies at; slide and shove compose, the move tween drives the rest).
