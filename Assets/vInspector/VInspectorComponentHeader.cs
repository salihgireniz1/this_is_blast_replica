        ...
        public void Update()
        {
            ...
            void findHeader(VisualElement element)
            {
                ...
                if (element.GetType().Name == "EditorElement")
                {
                    ...
                    {
                        // ponytail: Unity 6000.5 wraps the header IMGUIContainer in an UNNAMED VisualElement,
                        // so the old direct-child lookup found nothing and hoveredComponentHeader stayed null
                        // (killing the E / Shift-E / A / X shortcuts). Candidates now include one level down;
                        // the "Header" name check still runs on the IMGUIContainer itself.
                        // ponytail: one level only - if Unity nests it deeper later, make this a recursive search.
                        curHeaderImguiContainer ??= new[] { child as IMGUIContainer }
                            .Concat(child.Children().OfType<IMGUIContainer>())
                            .FirstOrDefault(r => r != null && r.name.EndsWith("Header"));
