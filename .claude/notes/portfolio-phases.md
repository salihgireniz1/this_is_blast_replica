# Portfolio phase plan, progress

Moved out of `CLAUDE.md` on 2026-09-04. Phases 0 and 1 of the portfolio plan (`Docs/PLAN.md`) as built before the case arrived: every chunk, the test-cull reasoning, the naming decision (`Cell` vs `Cube`), the deferred boot-scene trigger, the measurements from the original level bundle, the palette rules. The case overrides it wherever they disagree. Resume here after the case ships.
...
(The colour-space investigation and the original-game measurements that sat between 0.8 and 0.9 live in `colour-space-and-original-game.md`.)
